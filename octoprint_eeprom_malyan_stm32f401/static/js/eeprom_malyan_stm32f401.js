 ...
 * Modified by Matthew Upp on 29.05.2020
 */
$(function() {
    function EepromMalyanViewModel(parameters) {
        ...
        self.firmwareRegEx = /VERSION:[\s]*([^\s]*)[\s]PROTOCOL_VERSION:1.0[\s]MACHINE_TYPE:([^\s]*)/i; //[\s]*([^\s]*)[\s]*([^\s]*)/i;
        self.malyanRegEx = /Malyan[^\s]*/i; ///Malyan 3D Printer[^\s]*/i;

        //This plugin has not been thoroughly tested with any printer or firmware
        self.testedVersionRegEx = /00/ ///24|70|71|72|73|74/;

        self.setRegExVars('lastest');

        self.isMalyanFirmware = ko.observable(false);

        //This is only used to display a warning if the version is untested mine
        ...
        self.onStartup = function() {
            $('#settings_plugin_eeprom_malyan_stm32f401_link a').on('show', function(e) {
        ...
        self.fromHistoryData = function(data) {
            _.each(data.logs, function(line) {
                ...
                if (match !== null) {
                    self.FIRMWARE_NAME(match[2] + ' ' + match[1]);
                    ...
                    if (self.malyanRegEx.exec(match[0])) {
                        ...
                        if (self.testedVersionRegEx.exec(match[1]))
    ...
    OCTOPRINT_VIEWMODELS.push([
        ...
        "#settings_plugin_eeprom_malyan_stm32f401"
    ]);
});
